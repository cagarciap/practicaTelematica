	...
	def envioMensaje(self):
		if (len(self.consumidoresConectados) != 0):
			...
			for cliente in self.consumidoresConectados:
				...
				print(f'Cola grande: {self.canales[int(self.consumidoresConectados[idCliente][3])].getCola()}')
				print(f'Cola Consumidor: {self.consumidoresConectados[idCliente][4]}')
				print(f'IdConsumidor: {idCliente}')
				auxIndex = 0
				...
				value = value + 1
				"""while len(self.consumidoresConectados[idCliente][4]) != 0:
					respuesta = ""
					mensajeEnviar = self.consumidoresConectados[idCliente][4].popleft()
					print(mensajeEnviar)
					respuesta = f"Repsuesta para: {direccionAplicacion} Tiene un nuevo mensaje: {mensajeEnviar}\n"
					conexionAplicacion.sendall(respuesta.encode(constants.ENCODING_FORMAT))"""
		...
		"""if (len(self.consumidoresConectados) != 0):
			for cliente in self.consumidoresConectados:
				...
				nombre = arreglo[2]
				idCanal = arreglo[3]
				#colaCliente = arreglo[4]
				if (self.canales[int(idCanal)].getTamañoCola() != 0):
					colaCanal = self.canales[int(idCanal)].getCola()
					self.consumidoresConectados[idCliente][4] = colaCanal
					contadorAux = 0
					#print(len(self.consumidoresConectados[idCliente][4]))
					while contadorAux < len(self.consumidoresConectados[idCliente][4]):
						respuesta = ""
						mensajeEnviar = self.consumidoresConectados[idCliente][4].popleft()
						respuesta = f"Repsuesta para: {direccionAplicacion} Tiene un nuevo mensaje: {mensajeEnviar}\n"
						conexionAplicacion.sendall(respuesta.encode(constants.ENCODING_FORMAT))
	...
	def threaded(self, conexionAplicacion, direccionAplicacion):
	    while True:
	        ...
	        if (opcion == "SALIR"):
	        	...
	        	respuesta = f'Respuesta para: {direccionAplicacion[0]} El canal fue creado correctamente\n con el nombre {arreglo[1]} No olvide el token de identificacion del canal es: {self.contadorCanales}\n'
	        	...
	        	if (path.exists(os.getcwd()+"/usuarios.txt")):
	        		file = open(os.getcwd()+"/usuarios.txt", "r")
	        		lineas = file.readlines()
		        	if (usuario+"\n" in lineas):
		        		respuesta = f'Respuesta para: {direccionAplicacion[0]} El usuario ya existe\n'
		        		conexionAplicacion.sendall(respuesta.encode(constants.ENCODING_FORMAT))
		        	else:
		        		file = open(os.getcwd()+"/usuarios.txt", "a")
		        		file.write(usuario+"\n"+contrasena+"\n")
		        		file.close()
			        	respuesta = f'Respuesta para: {direccionAplicacion[0]} Sesión Creada\n'
			        	conexionAplicacion.sendall(respuesta.encode(constants.ENCODING_FORMAT))
	        	else:
	        		file = open(os.getcwd()+"/usuarios.txt", "w")
	        		file.write(usuario+"\n"+contrasena+"\n")
	        		file.close()
		        	respuesta = f'Respuesta para: {direccionAplicacion[0]} Sesión Creada\n'
		        	conexionAplicacion.sendall(respuesta.encode(constants.ENCODING_FORMAT))
